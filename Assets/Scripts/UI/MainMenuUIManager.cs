...
public class MainMenuUIManager : MonoBehaviour
{
    ...
    public async Promise<CreateLobbyDetails> CreateLobbyAsync(CreateLobbyRecord lobbyRecord)
    {
        ...
        // Create a Lobby
        /*
        HathoraCloud.Models.Operations.CreateLobbyRequest createLobbyRequest = new HathoraCloud.Models.Operations.CreateLobbyRequest()
        {
            AppId = _clientConfig.AppId,
            CreateLobbyV3Params = new CreateLobbyV3Params()
            {
                Region = Region.Chicago,
                Visibility = lobbyRecord.Restriction == LobbyRestriction.Open ? LobbyVisibility.Public : LobbyVisibility.Private,
        ...
        */
        ...
        /*
        CreateLobbySecurity security = new CreateLobbySecurity()
        {
            PlayerAuth = HathoraClientSession.Singleton.PlayerAuthToken,
        };

        CreateLobbyResponse res = await _hathora.LobbyV3.CreateLobbyAsync(security, createLobbyRequest);

        // Check if lobby was created successfully
        if (res.StatusCode != 201)
        {
            Debug.LogError("Failed to create Hathora lobby: " + res.StatusCode + "\n" + res.ApiError.Message);
            _serverStatusText.text = "Failed to create lobby";
            return null;
        }

        _serverStatusText.text = "Getting lobby connection info...";

        GetConnectionInfoResponse roomRes = await _hathora.RoomV2.GetConnectionInfoAsync(new GetConnectionInfoRequest()
        {
            AppId = _clientConfig.AppId,
            RoomId = res.LobbyV3.RoomId,
        });

        // Check if the room was created successfully
        if (roomRes.StatusCode != 200)
        {
            Debug.LogError("Failed to get Hathora room connection info: " + roomRes.StatusCode + "\n" + roomRes.ApiError.Message);
            _serverStatusText.text = "Failed to get room connection info";
            return null;
        }
        */
        ...
        var lobbyApi = _beamContext.ServiceProvider.GetService<IBeamLobbyApi>();
